...
<!--
	...
	- hasAnyRole([role,role2]): 여러 roles 중에서도 하나라도 해당하는 롤이 있으면 true(or처럼)
	...
	- isFullyAuthenticated(): Remember-me(로그인 유지)로 인증된 것이 아닌 일반적인 방법으로 인증된 사용자인 경우 true
 -->
		...
		<div style="float: right;">
			...
			<sec:authorize access="isAuthenticated()">
				<form action="/logout" method="post">
					<button type="submit" onclick="fn_submit()"
						class="btn btn-outline-light">로그아웃</button>
					<sec:csrfInput />
				</form>
				<button type="button" class="btn btn-outline-warning btn-sm"
					data-bs-toggle="modal" data-bs-target="#exampleModal"
					data-bs-whatever="@mdo">나의 정보 보기</button>
			</sec:authorize>
		</div>
	</div>
</nav>
