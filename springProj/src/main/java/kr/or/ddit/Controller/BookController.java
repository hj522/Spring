...
public class BookController {
	...
	@Autowired
	BookService bookService;	// 스프링은 인터페이스를 좋아해
	...
	// 1) 요청 URI(모든 책) => http://localhost/list
	// 2) 요청 URI(검색하는 경우) => http://localhost/list?keyword=개똥
	// 골뱅이RequestParam(value="파라미터name(keyword)", required=false(?keyword=일 때 오류방지))
	...
	// 책 변경
	// http://localhost/update?bookId=2&title=개똥이월드2&category=소설&price=12000
